...
class ConcurrentSOMDManager:
    ...
    def wait_for_lambda_window(self, lambda_str: str) -> bool:
        """Wait for all runs of a specific lambda window to complete"""
        self.logger.info(f"Waiting for all runs of lambda {lambda_str} to complete...")
        
        # Find all jobs for this lambda
        lambda_jobs = [
            (job_id, future) for job_id, future in self.futures.items()
            if self.job_metadata[job_id]['lambda_str'] == lambda_str
        ]
        
        # Wait for completion
        for job_id, future in lambda_jobs:
            try:
                result = future.result()  # This blocks until complete
                self.logger.info(f"Lambda {lambda_str} job {job_id} completed")
            except Exception as e:
                self.logger.error(f"Lambda {lambda_str} job {job_id} failed: {e}")
                return False
        
        # Verify no runs are still tracked as running
        with self.lambda_locks[lambda_str]:
            still_running = len(self.lambda_runs[lambda_str])
            if still_running > 0:
                self.logger.warning(f"Lambda {lambda_str} still has {still_running} runs tracked as running")
                return False
        
        self.logger.info(f"✅ All runs for lambda {lambda_str} completed successfully")
        return True
